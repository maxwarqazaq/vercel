...
def set_webhook():
    try:
        ...
        print(f"Error in set_webhook: {str(e)}")
...
/download <message_id> - Get a file's download URL (up to 20MB due to Telegram API limits).
...
/savefile <message_id> - Log file details for saving (up to 20MB).
                """
...
3. Use /download <message_id> to get a download URL (up to 20MB).
                """
                ...
                try:
                    ...
                    if (message_id in uploaded_files and 
                        ...
                        if file_size > MAX_FILE_SIZE_MB * 1024 * 1024:
                            send_message(chat_id, f"File is too large ({file_size / (1024*1024):.2f}MB). Telegram API limits downloads to {MAX_FILE_SIZE_MB}MB. Access it directly at: https://t.me/{CHANNEL_USERNAME[1:]}/{message_id}")
                            return jsonify({"status": "processed"}), 200
                        ...
                        if file_data and file_data.get("ok"):
                            ...
                            send_message(chat_id, f"Download URL (valid for 1 hour): {download_url}\nNote: Bot can’t download files over 20MB due to Telegram API limits.")
                        else:
...
- Total Unique Users: {total_users}
        ...
        if file_id:
            ...
            if file_size_bytes > UPLOAD_LIMIT_MB * 1024 * 1024:
                send_message(chat_id, f"File too large ({file_size_bytes / (1024*1024):.2f}MB). Max upload size is {UPLOAD_LIMIT_MB}MB.")
                return jsonify({"status": "processed"}), 200
